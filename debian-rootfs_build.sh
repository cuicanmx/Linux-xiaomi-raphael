...
BASE_PACKAGES="systemd udev dbus bash-completion net-tools systemd-resolved wpasupplicant iw iproute2 sudo openssh-server openssh-client chrony vim wget curl iputils-ping zstd network-manager wireless-regdb alsa-ucm-conf alsa-utils initramfs-tools u-boot-tools ca-certificates"
...
chroot rootdir apt install -y $BASE_PACKAGES || { echo "安装包失败" >&2; exit 1; }
chroot rootdir systemctl enable chrony

# 安装桌面
if [[ "$DISTRO_VARIANT" == "desktop" ]]; then
    if [[ "$DISTRO_TYPE" == "debian" ]]; then
        chroot rootdir apt install -y task-gnome-desktop || true
    else
