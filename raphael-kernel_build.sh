...
# 更新 control 文件版本
log_config "更新 control 文件版本..."
sed -i "s/Version:.*/Version: ${_kernel_version}/" ../linux-xiaomi-raphael/DEBIAN/control

# 清理和安装模块
log_build "清理和安装模块..."
rm -rf ../linux-xiaomi-raphael/lib
make -j$(nproc) ARCH=arm64 CROSS_COMPILE="ccache aarch64-linux-gnu-" INSTALL_MOD_PATH=../linux-xiaomi-raphael modules_install
...
log_with_time "$LOG_LEVEL_SUCCESS" "$LOG_TYPE_END" "内核编译完成，结束时间: $(date)"